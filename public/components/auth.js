...
export class AuthComponent {
  ...
  async checkAuthStatus() {
    ...
    // Check if there's a sessionId in localStorage (backend-based auth)
    const storedSessionId = localStorage.getItem('sessionId');
    if (storedSessionId) {
      console.log('Found stored sessionId, validating:', storedSessionId);
      await this.validateSession(storedSessionId);
      return;
    }

    // Try MSAL silent token or handle redirect return (only for /beta)
    // Skip MSAL for main app to avoid unwanted popups
    const isMainApp = !window.location.pathname.includes('/beta');
    if (isMainApp) {
      console.log('Main app detected, skipping MSAL and showing login modal');
      this.render();
